...
async function getValidTokens() {
  ...
  if (!tokens) {
    throw new Error(
      'No tokens found. Run `npm run upwork:auth` first to complete OAuth consent.'
    );
  }
  if (isExpired(tokens)) {
    await refreshToken();
...
export async function refreshToken() {
  const { clientId, clientSecret } = await loadUpworkConfig();
  const tokens = await loadTokens();
  if (!tokens?.refresh_token) {
    throw new Error('No refresh_token — run `npm run upwork:auth` to authorize again.');
  }
