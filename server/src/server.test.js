...
test('protects the server and exposes feature-scoped actions', async () => {
  await withServer(async (baseUrl) => {
    ...
    assert.deepEqual(payload.actions.map(({ id, method, path }) => ({ id, method, path })), [
      ...
      { id: 'planning.focus.read', method: 'GET', path: '/api/planning/focus' },
      { id: 'planning.focus.item.create', method: 'POST', path: '/api/planning/focus/items' },
      { id: 'planning.focus.item.update', method: 'PATCH', path: '/api/planning/focus/items/{itemId}' },
      { id: 'planning.focus.item.delete', method: 'DELETE', path: '/api/planning/focus/items/{itemId}' },
      { id: 'planning.focus.reorder', method: 'POST', path: '/api/planning/focus/reorder' },
