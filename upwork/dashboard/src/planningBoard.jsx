      ...
      <Dialog open={projectDialog}>
        <DialogContent className="planning-dialog">
          <form onSubmit={submitProject}>
            <DialogHeader className="planning-dialog-header"><button type="button" className="dialog-close" onClick={() => setProjectDialog(false)}><X /></button><span className="section-kicker">Local project</span><DialogTitle>{editingProject ? 'Project settings' : 'Link a directory'}</DialogTitle><p>The path is validated on this Mac and becomes the working directory in every agent brief.</p></DialogHeader>
            <DialogBody className="planning-form">
              ...
              <Field label="Directory on disk" hint="Choose a folder or enter an absolute path."><div className="path-input"><Folder /><Input required value={projectForm.directory} onChange={(event) => setProjectForm({ ...projectForm, directory: event.target.value })} placeholder="/Users/you/Projects/my-project" /><button type="button" className="choose-directory-button" onClick={chooseProjectDirectory} aria-label="Choose directory" title="Choose directory"><FolderOpen /></button></div></Field>
